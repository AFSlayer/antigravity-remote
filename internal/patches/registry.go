...
func All() []Patch {
	return []Patch{
		...
		},
		{
			...
			Replace: `var WS=({name:a,path:b,onCopyPath:c,description:d,badge:f,disabled:g=!1,isLast:h=!1,onEdit:k,editTitle:l="Edit",onDelete:m,deleteTitle:n="Delete",onToggle:p,toggleChecked:r,toggleDisabled:t=!1,expandableContent:v})=>{var [agyEdit,agySetEdit]=(0,G.useState)(!1),[agyTxt,agySetTxt]=(0,G.useState)(""),[agySave,agySetSave]=(0,G.useState)(!1),[agyDone,agySetDone]=(0,G.useState)(!1);var agyDoEdit=k||(b?async()=>{if(agyEdit){agySetEdit(!1);return;}try{let res=await fetch("/__agy/api/rules/read?path="+encodeURIComponent(b));if(res.ok){let json=await res.json();agySetTxt(json.content||"");agySetEdit(!0);}}catch(e){console.error(e)}}:void 0);var agyDoSave=async()=>{agySetSave(!0);try{let res=await fetch("/__agy/api/rules/save",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({path:b,content:agyTxt})});if(res.ok){agySetDone(!0);setTimeout(()=>{agySetDone(!1);agySetEdit(!1)},1000);}}catch(e){console.error(e)}finally{agySetSave(!1)}};k=agyDoEdit;var agyExp=v||(agyEdit?G.createElement("div",{className:"w-full mt-2 pt-2 border-t border-border flex flex-col gap-2"},G.createElement("textarea",{value:agyTxt,onChange:e=>agySetTxt(e.target.value),placeholder:"Write rule markdown instructions...",className:"w-full font-mono text-xs p-2.5 rounded-lg border border-border bg-muted/40 focus:outline-none focus:ring-1 focus:ring-primary min-h-[220px] max-h-[500px] resize-y text-foreground leading-relaxed",spellCheck:!1}),G.createElement("div",{className:"flex items-center justify-end gap-2"},G.createElement(Ky,{variant:"ghost",size:"sm",onClick:()=>agySetEdit(!1),disabled:agySave,className:"text-xs h-7 px-3"},"Cancel"),G.createElement(Ky,{variant:"secondary",size:"sm",onClick:agyDoSave,disabled:agySave,className:"text-xs h-7 px-3 flex items-center gap-1 font-medium bg-primary text-primary-foreground hover:bg-primary/90"},agySave?"Saving...":(agyDone?"Saved ✓":"Save Rule 💾")))):(agyEdit?G.createElement("div",{className:"w-full mt-2 pt-2 border-t border-border flex flex-col gap-2"},G.createElement("textarea",{value:agyTxt,onChange:e=>agySetTxt(e.target.value),placeholder:"Write rule markdown instructions...",className:"w-full font-mono text-xs p-2.5 rounded-lg border border-border bg-muted/40 focus:outline-none focus:ring-1 focus:ring-primary min-h-[220px] max-h-[500px] resize-y text-foreground leading-relaxed",spellCheck:!1}),G.createElement("div",{className:"flex items-center justify-end gap-2"},G.createElement(Ky,{variant:"ghost",size:"sm",onClick:()=>agySetEdit(!1),disabled:agySave,className:"text-xs h-7 px-3"},"Cancel"),G.createElement(Ky,{variant:"secondary",size:"sm",onClick:agyDoSave,disabled:agySave,className:"text-xs h-7 px-3 flex items-center gap-1 font-medium bg-primary text-primary-foreground hover:bg-primary/90"},agySave?"Saving...":(agyDone?"Saved ✓":"Save Rule 💾")))):void 0);v=agyExp;var w=k||m||p,`,
		},
